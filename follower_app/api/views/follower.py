...
class FollowView(GenericViewSet,CreateModelMixin, DestroyModelMixin):
    ...
    @action(methods=['get', ], detail = False, serializer_class=UserSerializer, url_path=r'(?P<user_id>[^/.]+)/followings')
...
class FollowHashtagView(GenericViewSet,CreateModelMixin, DestroyModelMixin):
    ...
    @action(methods=['get', ], detail = False, serializer_class=HashtagSerializer, url_path=r'(?P<user_id>[^/.]+)/following_tags')
    def user_followings(self, request, *args, **kwargs):
        user_id = self.kwargs['user_id']
        if user_id is not None:
